...
namespace Foroffer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Category(int Id)
        {
            ...
            subModel.Posts = _offerDbContext.Posts.Where(y => y.Subcategory.CategoryId == Id).OrderByDescending(y => y.Id).ToList();
            subModel.Companies = _offerDbContext.Companies.ToList();
            ViewBag.Posts = _offerDbContext.Posts.Where(u => u.Subcategory.CategoryId == Id).GroupBy(u => new { u.Company.Name }).Select(u => u.First()).ToList();
            ViewData["Category"] = _localizer["Kateqoriyalar"];
